...
| webview CSP 对 blob / data URL 的限制 | ✅ **已验证（M1）** | 媒体显示 | 见 §12.1；风险解除 |
| 34 个厂商模板无法逐个真机验证 | 已知限制（#5） | #5 的可信度 | #5 设计时标注"未验证"并给出探活机制 |

### 12.1 M1 探针实测结论（2026-09-19）

在真实 VSCode 扩展开发宿主中点击侧边栏，探针页面 9 项自检**全部通过**：

| # | 自检项 | 结果 |
|---|---|---|
| 1 | 注入的媒体映射条目数 | ✅ 1 |
| 2 | 媒体 URL 被改写为映射目标（`img.src` 已指向 `asWebviewUri` 地址） | ✅ |
| 3 | 图片实际加载成功（CSP 允许 `cspSource` 资源） | ✅ 96×96 |
| 4 | `fetch` 返回 `Response` 实例 | ✅ |
| 5 | `response.status` 可读 | ✅ 200 |
| 6 | `response.ok` 语义正确 | ✅ |
| 7 | `response.json()` 可解析 | ✅ |
| 8 | `PUT` 的 method 与 body 正确透传 | ✅ |
| 9 | 未注册路由返回 404 而非抛错 | ✅ |

**这意味着什么：** 本项目最大的架构赌注已被证实——“上游 14949 行的前端脚本能否不改一行地搬进
VSCode webview”答案为**可以**：#1 可以整文件搬运上游 `assets/whale-widget.js`，
只需改写其开头的“DSH 页面自检”与挂载入口，而不必重写泡泡系统等重型 UI。

**因此解除的风险：**

- webview CSP 对媒体资源的限制——`asWebviewUri` 路径畅通
- 分治桥的两条通道（数据过消息通道、媒体走原生文件服务）——均成立
- shim 合成 `Response` 的语义完整性——`.ok` / `.status` / `.json()` 均与真实 HTTP 响应一致

**仍然待验证：** `asWebviewUri` 加载的音频是否支持 HTTP range（只影响 #4 的音频拖动，
#0–#3 不受影响）。
